/* A special resizing array that supports the below operations in O(1) or 
   amortised O(1) time:
   ...
   In order to support remove and contains in O(1), we use a hash table.

   insert – Everytime an item is inserted, we record that item's position in the
   hash table. The hash table thus stores the key value pair of (item, position).

   remove – To remove the given item in O(1) time, we need to know its position.
   This position is already recorded in the hash table.
   (i)  We therefore retrieve the position first from the hash table in O(1)
        amortised time.
   (ii) Removing the item from its current position means setting it to null.
        This will however create a gap in the array. To prevent this, we fill
        the gap with the last item in the array and take care of the last-index
        accordingly.
   
   The remove operation highlights the two limitations:
   (i)  duplicates are not allowed in the data structure
   (ii) ordered operations are not suppored (this is implied anyway since it is 
        not listed in the API.

   contains – Since every insert adds the value of the item as a key in the hash
   table, we are able to take advantage of the hash table's O(1) amortised
   look-up operation to support this in O(1) (amortised).
...
public class SpecialResizingArray<Item> implements Iterable<Item>
{
    ...
    private Hashtable<Item, Integer> positions; // stores the position of the 
                                                // given value in the values array

    private int capacity;  // capacity of the values array
    private int nextItemIndex; // index at which the next value will be added;
                               // can also be used to query the number of elements, 
                               // aka (size), since the array is zero-indexed.
    ...
    public SpecialResizingArray() { this(DEFAULT_CAPACITY); }
    ...
    // arbitrary order
    public Iterator<Item> iterator() { return new SpecialResizingArrayIterator(); }
    ...
    private class SpecialResizingArrayIterator implements Iterator<Item>
    {
        ...
        public SpecialResizingArrayIterator() { index = 0; }
